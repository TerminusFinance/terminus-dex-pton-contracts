...
export function preprocBuildContracts(opts: { autocleanup?: boolean; data?: Data }): void {
    const folderPath = 'contracts';
    if (opts.autocleanup === undefined) opts.autocleanup = true;

    cleanupBuild(folderPath);
    if (opts.autocleanup) process.on('exit', (_) => cleanupBuild(folderPath));
