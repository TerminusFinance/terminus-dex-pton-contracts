...
export function getContractCode(contract: string): Cell {
    ...
    try {
        ...
        let msg = color.colorText(
            `<bld><r>Error: build data for <b>'${contract}' <r>cannot be found, you need to build it first`,
        );
...
export async function compileX(
    contract: string,
    params?: {
        noSave?: boolean;
        cells?: boolean;
...
export function createInternalMsgCell(params: {
    ...
    let msg = beginCell()
        ...
        .storeUint(
            (params.payload instanceof Cell ? 1 : 0) + (params.stateInit ? 6 : 0),
            107 + (params.stateInit ? 1 : 0),
        );
...
export async function getWalletBalance(
    wallet: SandboxContract<JettonWalletContract> | OpenedContract<JettonWalletContract>,
) {
    try {
...
export function parseVersion(): [number, number, number, string] {
    try {
        ...
        let dev = sub.length > 1 ? sub[1] : Number(num[2]) === 0 ? 'release' : `patch${num[2]}`;
