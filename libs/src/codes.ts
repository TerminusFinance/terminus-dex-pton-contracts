...
export function parseErrors(path: string) {
    ...
    for (let item of entries) {
        ...
        if (typ != 'error') throw new Error(`type '${typ}' is not an error`);
        if (key in res) throw new Error(`key '${key}' defined twice`);
        if (!Number.isInteger(value)) throw new Error(`value '${value}' is not a number`);
        if (Object.values(res).includes(value)) throw new Error(`value '${value}' defined twice`);
