...
export function mintMsgConfigToCell(config: MintMsgConfig): Cell {
    ...
    if (config.payload instanceof Cell) {
        res.storeUint(1, 1).storeRef(config.payload);
    } else if (config.payload instanceof Slice) {
        res.storeUint(0, 1).storeSlice(config.payload);
    } else {
        res.storeUint(0, 1);
    }
...
export function metadataCell(content: string | Dictionary<Buffer, Cell>): Cell {
    ...
    if (typeof content === 'string') {
        res = beginCell().storeUint(0x01, 8).storeStringTail(content).endCell();
    } else {
        res = beginCell().storeUint(0x00, 8).storeDict(content).endCell();
    }
    return res;
}
...
export abstract class JettonMinterContractBase<T extends JettonMinterOpcodesType> extends CommonContractBase {
    ...
    async sendMint(
        ...
        if (!this.opCodes.mint) throw new Error('Not Implemented');

        value = opts.value ?? value;
        if (!value) throw new Error('Message must have value');

        const mstMsg =
            opts.masterMsg instanceof Cell
                ? opts.masterMsg
                : mintMsgConfigToCell({
                      ...opts.masterMsg,
    ...
    async sendBurnNotification(
        ...
        if (!this.opCodes.burnNotification) throw new Error('Not Implemented');
    ...
    async sendChangeAdmin(
        ...
        if (!this.opCodes.changeAdmin) throw new Error('Not Implemented');
        ...
        await provider.internal(via, {
            value: value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginMessage(this.opCodes.changeAdmin).storeAddress(opts.newAdminAddress).endCell(),
        });
    }
    ...
    async sendChangeContent(
        ...
        if (!this.opCodes.changeContent) throw new Error('Not Implemented');
        ...
        await provider.internal(via, {
            value: value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginMessage(this.opCodes.changeContent).storeRef(opts.content).endCell(),
        });
    }
