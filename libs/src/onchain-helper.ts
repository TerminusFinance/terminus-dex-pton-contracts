...
export function prettyBalance(
    ...
    tokenData: AsyncReturnType<typeof fetchJettonData>,
) {
...
export async function waitSeqNoChange(
    ...
    if (!timeout) throw new Error('timeout must be greater than 0');
...
export async function fetchJettonData(
    jetton: OpenedContract<JettonMinterContract>,
    removeRaw = true,
): Promise<Optional<JettonContent & JettonData & { decimals: number }, 'contentRaw' | 'jettonWalletCode' | 'content'>> {
