...
export function flattenTransactionExtended(tx: BlockchainTransaction) {
    ...
    if (description.type === 'generic' && description.actionPhase) {
        ...
        res.totalActionFee = description.actionPhase.totalActionFees
            ? description.actionPhase.totalActionFees
            : undefined;
    }
...
export const defaultCaptionMap: Map<number, CaptionHandler> = new Map(
    opEntries({
...
export function createMdGraph(params: GraphArgsType) {
    ...
    const mapKey = (address: Address | -1, isTo = false) => {
        const addrStr = address.toString();
        if (!internalMap.has(addrStr) || (isTo && paramsX.directionType === 'unidirectional')) {
    ...
    const createLink = (tx: FlatTransactionExtended, ind: number) => {
        ...
        let txInfo: string[] = [];
        const addInfo = (label: string, data: any) => {
            txInfo.push(`${label}: ${data}`);
        };
        // ------------------------------
        if (paramsX.displayIndex) {
            addInfo('index', ind);
        }
        // ------------------------------
        if (paramsX.displayValue && typeof tx.value !== 'undefined') {
            addInfo('value', fromNanos(tx.value));
        }
        // ------------------------------
        ...
        // ------------------------------
        if (paramsX.displayTokens && typeof tx.op !== 'undefined' && typeof tx.body !== 'undefined') {
        ...
        // ------------------------------
        if (paramsX.displayExitCode && typeof tx.exitCode !== 'undefined' && (!paramsX.hideOkValues || tx.exitCode)) {
        ...
        // ------------------------------
        if (paramsX.displayDeploy && (!paramsX.hideOkValues || tx.deploy)) {
            addInfo('deploy', tx.deploy);
        }
        // ------------------------------
        if (paramsX.displayAborted && typeof tx.aborted !== 'undefined' && (!paramsX.hideOkValues || tx.aborted)) {
        ...
        // ------------------------------
        if (paramsX.displaySuccess && typeof tx.success !== 'undefined' && (!paramsX.hideOkValues || tx.success)) {
