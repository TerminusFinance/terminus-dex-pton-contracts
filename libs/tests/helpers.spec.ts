...
;; exit codes`);
        ...
        const entries2 =
            parser.parse(`const op::ton_transfer    = "ton_transfer query_id:uint64 ton_amount:Coins refund_address:MsgAddress forward_payload:(Either Cell ^Cell) = InternalMsgBody"c & 0x7fffffff;
...
const error::wrong_op = 0xFFFF;`);
    ...
    it('should test getContractCode', async () => {
        ...
        fs.mkdirSync('build', { recursive: true });
        fs.writeFileSync(
            `build/_test_getContractCode.compiled.json`,
            JSON.stringify({
                hex: boc,
            }),
        );
    ...
    it('should test getWalletBalance', async () => {
        ...
        const minter = blockchain.openContract(
            JettonMinterContract.createFromConfig(config, DEFAULT_JETTON_MINTER_CODE),
        );
        ...
        let wallet = blockchain.openContract(
            JettonWalletContract.createFromAddress(await minter.getWalletAddress(deployer.address)),
        );
    ...
    it('should test findArgs', async () => {
        let testArr = ['123', '456', 'test1', 'test2', '789'];
